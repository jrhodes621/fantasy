...
task :create_rosters_nfl => [:environment] do
  ...
  all_goalies = players.select { |player| player[:position] == "TE"}.sort_by { |v| v[:points] }.reverse.take(30)
    ...
    for w in (0..all_wingers.count).step(size)
      for d in (0..all_defencemen.count).step(size)
        for te in (0..all_goalies.count).step(size)
          ...
          goalies = all_goalies.drop(te).take(size)
...
def process_nfl_rosters center_combos, winger_combos, defencemen_combos, goalies_combos, min_points
  ...
  products.each do |product|
    ...
    next if(salary1 > 47000 || salary1 < 45000)
